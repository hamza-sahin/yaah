...
- `implementer` — which engine runs the Phase 3 build and every Phase 4 fix round. `implementer.engine` is `claude` (default — a Claude Code subagent via the Agent tool), `cursor` (the `cursor-agent` CLI), or `codex` (the OpenAI `codex exec` CLI); the two CLI engines run headlessly inside the worktree. `implementer.model` (optional) overrides the model: for **claude** it is an Agent-tool alias (`sonnet`/`opus`/`haiku`, blank = inherit the session model); for **cursor/codex** it is that engine's own model id (blank = that CLI's default; ids are engine-specific). Two more keys apply to the **claude** engine only (cursor/codex ignore them): `implementer.agent` — the subagent type forge invokes (default `general-purpose`; must have Bash + tracker CLI + edit tools); and `implementer.workflow` — `false` (default) builds the children with one sequential subagent, `true` builds dependency-independent children **in parallel** via the Workflow tool (one isolated worktree per agent, integrated onto the single branch/PR). A missing block = `claude` / `general-purpose` / `workflow:false`. Only the *implementer* changes; grilling, issues, review, and merge are identical across engines. Exact commands + preflight live in [PLAYBOOK.md](PLAYBOOK.md) under "Implementer engines".
